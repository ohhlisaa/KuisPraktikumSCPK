...
% Last Modified by GUIDE v2.5 24-Apr-2021 22:20:41
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
opts = detectImportOptions('StatusGizi.csv');
opts.SelectedVariableNames = (1:5);
data = readmatrix('StatusGizi.csv', opts);
...
edit5 = str2double(get(handles.edit5, 'String'));

sample = [edit1 edit2 edit3 edit4 edit5]

opts = detectImportOptions('StatusGizi.csv');
opts.SelectedVariableNames = (1:5);
training = readmatrix('StatusGizi.csv', opts);

opts = detectImportOptions('StatusGizi.csv');
opts.SelectedVariableNames = (6);
group = readmatrix('StatusGizi.csv', opts);

class = fitcknn(training, group, 'NumNeighbors', 3);
klasifikasi = predict(class, sample);

set(handles.text7, 'String', klasifikasi);
